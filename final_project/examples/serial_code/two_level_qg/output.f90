...
CONTAINS
  ...
  SUBROUTINE WRITE_OUTPUT(step, time, dt)
    ...
    CHARACTER(LEN = 33) :: level1FileName !< Output file name for level 1.
    CHARACTER(LEN = 33) :: level2FileName !< Output file name for level 2.
    CHARACTER(LEN = 39) :: timestepInfoFileName !< Output file name for
    !! time-step information.

    WRITE(level1FileName,'(A,I0.8,A)') './output_data/level1_', step, '.csv'
    WRITE(level2FileName,'(A,I0.8,A)') './output_data/level2_', step, '.csv'
    WRITE(timestepInfoFileName,'(A,I0.8,A)') './output_data/out_', step, &
         & '_info.txt'
