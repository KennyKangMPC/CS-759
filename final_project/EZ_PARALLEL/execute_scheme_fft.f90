...
SUBROUTINE EXECUTE_SCHEME_FFT_DBLE_SBR(subGrid, kind, sch)
  ...
  CALL EXECUTE_SCHEME_FFT_DBLE_EH(kind, sch)
  ...
  CASE(FFT_1D_1)
     ...
     DO j = sch%vSlabInt(0), sch%vSlabInt(1)
        CALL DFFTF(sch%vSlabSizeOvlp(0), subGrid(:,j), sch%WSAVE1)
     END DO
  ...
  CASE(FFT_1D_2)
     ...
     DO i = 0, sch%hslabSize(0)-1
        CALL DFFTF(sch%hSlabSize(1), subGridInt_T(i,:), sch%WSAVE2)
     END DO
...
CONTAINS
  ...
  END SUBROUTINE EXECUTE_SCHEME_FFT_DBLE_EH
...
SUBROUTINE EXECUTE_SCHEME_FFT_DCMPX_SBR(subGrid, kind, sch)
  ...
  ! Check for errors in user input.
  CALL EXECUTE_SCHEME_FFT_DCMPX_EH(kind, sch)
  ...
  CASE(FFT_1D_1)
     ...
     ! FFT the columns.
     DO j = sch%vSlabInt(0), sch%vSlabInt(1)
        CALL ZFFTF(sch%vSlabSizeOvlp(0), subGrid(:,j), sch%WSAVE1)
     END DO
...
CONTAINS
  ...
  END SUBROUTINE EXECUTE_SCHEME_FFT_DCMPX_EH
