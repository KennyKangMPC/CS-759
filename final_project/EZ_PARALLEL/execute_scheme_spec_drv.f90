...
SUBROUTINE EXECUTE_SCHEME_SPEC_DRV_DBLE_SBR(subGrid, kind, order, sch)
  ...
  CALL EXECUTE_SCHEME_SPEC_DRV_DBLE_EH(kind, order, sch)

  IF (sch%procID .EQ. 0) THEN
     PRINT *, "ERROR: Spectral derivatives for DOUBLE PRECISION grids is ", &
          "not yet supported."
  END IF
...
CONTAINS
  ...
  END SUBROUTINE EXECUTE_SCHEME_SPEC_DRV_DBLE_EH
...
SUBROUTINE EXECUTE_SCHEME_SPEC_DRV_DCMPX_SBR(subGrid, kind, order, sch)
  ...
  ! Check for errors in user input.
  CALL EXECUTE_SCHEME_SPEC_DRV_DCMPX_EH(kind, order, sch)
...
CONTAINS
  ...
  END SUBROUTINE EXECUTE_SCHEME_SPEC_DRV_DCMPX_EH
