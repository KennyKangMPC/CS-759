    ...
    static void writeSolutionFile(Cell[][] cells, double time, double cfl){
        ...
        try{
            try(FileWriter fileWriter = new FileWriter(file)){
                ...
                fileWriter.write("NumYCells:" + Info.NUM_Y_CELLS + "\n");
                fileWriter.write("NumGhostCells:" + Info.NUM_GHOST_CELLS + "\n");
                fileWriter.write("time:" + time + "\n");
                fileWriter.write("CFL:" + cfl + "\n");
                for (Cell[] cellRow : cells){
                    for (Cell cell : cellRow){
                        fileWriter.write(String.format("%20.15f %20.15f", cell.cx, cell.cy));
                        for (int var = 0; var < Cell.NUM_VARS; var++){
                            fileWriter.write(String.format("%20.15f", cell.U[rkStep][var]));
                        }
                        fileWriter.write("\n");
                    }
                }
